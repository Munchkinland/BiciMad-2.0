...
def visualizar_predicciones(fechas, predicciones, titulo):
    ...
    fig.update_layout(title=titulo, xaxis_title='Fecha 📆', yaxis_title='Demanda prevista biciMad 📊', template='plotly_dark')
    
    # Agregar botones interactivos para cambiar el período de tiempo
    buttons = []
    for periodo, dias in periodos.items():
        inicio = hoy
        fin = hoy + timedelta(days=dias)
        visible = [False] * len(periodos)
        visible[list(periodos.keys()).index(periodo)] = True
        button = dict(label=periodo,
                      method='update',
                      args=[{'visible': visible},
                            {'title': f'Demanda Predicha - {periodo}'}])
        buttons.append(button)
        
    fig.update_layout(updatemenus=[dict(buttons=buttons, direction="down", x=0.5, xanchor="center", y=1.15, yanchor="top")])
    fig.update_xaxes(rangeslider_visible=True, rangeselector=dict(buttons=list([dict(count=1, label="1m", step="month", stepmode="backward"), dict(count=6, label="6m", step="month", stepmode="backward"), dict(step="all")])))
    fig.show()
